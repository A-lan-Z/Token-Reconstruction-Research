...
The tested H−S intervals span zero: −0.0116 pp [−0.2431, +0.2199] on `public_base` and −0.2083 pp [−0.4282, +0.0347] on the target update. The tested S−affine intervals also span zero: +0.0231 pp [−0.2315, +0.2546] and +0.0347 pp [−0.2083, +0.2662]. No primary H−affine interval was tested. The study therefore stops this D objective and does not promote a teacher/ranking student or claim that all GRU or teacher methods fail. It retains the affine control and native A1+A2 accuracy anchor for context. P03's static/projected variant remains stopped and unmerged.
...
The separate anchor is 12 predeclared length-32 records (4 per style, 384 tokens per condition) and is a PR7-compatible numerical/input port of the native A1+A2 procedure. It retains PR7's K=512 proposal, K=256 candidate, 256-wide projection chunk, record-batch-one execution, published proposal order, and first-argmax behavior, while adapting the input to the P04 cut-depth-4 activation panel and 12-record × 32-post-BOS-position slice. Both target arms, all 16 student cells, and both native anchors were frozen before truth. The joint freeze was created at `2026-09-06T04:13:14Z`, has SHA-256 `3c8ff3ea0e3435ded45b3e2a75b0498fbd2eee9fce39ab409ec5a942b69417d7`, and records `truth_accessed=false`. Scoring completed at `2026-09-06T04:18:15Z` from score artifact SHA-256 `f9a604727eba9cd0f7e7bdb2a2c1e1ed828e15d9c9ae551b375a18c640277853`; prediction files were read before truth and not rewritten, and truth was opened only after the verified freeze.
...
For a compact length view, each entry is `D−H / D−S / H−S / S−affine` in percentage points, aggregated over both seeds. Each style×length cell has six independent source records with two paired seed predictions per source record (12 record-level predictions); order is 16, 32, 64, 128.
...
Native A1+A2 remains very strong: 384/384 tokens and 12/12 records on `public_base`, and 383/384 and 11/12 under the target update. The native procedure uses PR7 published proposal order and first-argmax behavior. The causal student readouts use lowest-token-ID tie handling on exact finite-precision ties; this is a deliberate tie-rule exception and does not imply that native A1+A2 used the student rule. The anchor is a numerical/input compatibility port for P04, so its current timing is not a historical GPU-equivalence claim.
...
The audit had finite scores throughout, 9/384 K=32 proposal misses and 1/384 K=512 misses, score scale `sigma_q=0.01452335`, tie tolerance `0.0001452335`, and 86 omitted near-tie pairs. The raw JSON field `a1_accuracy` means the frozen P04 affine proposer here, not historical A1. All three public styles were present, but the error-driven audit was dominated by `pile_plain` (313/384 positions), so it is qualification evidence rather than a fresh-panel balance claim. The student architecture uses a trainable full affine block in every arm, a GRU residual of width 256 whose output projection is zero-initialized in S/H/D, and one fixed normalized public embedding table tied across arms. The exact objectives are CE for S, CE + 0.25 × softplus hard-confusion margin loss (margin 1.0) for H, and CE + 0.25 × softplus hard-confusion margin loss + 0.25 × teacher-ranking loss for D. The ranking loss is averaged over retained non-gold pairs on teacher-qualified rows, not over all selected positions. This fixed result leaves an unresolved design question—whether D's loss scale or the teacher information itself caused the regression—which was not separated by a sweep or follow-up fit.
...
Public capture took 6.357 s; teacher qualification took 88.0877 s for 12,288 candidate simulations. Student full-panel timings are batch-8 throughput-derived per-record values, not single-record latency: affine 2.2176–2.2516 ms/record, S 2.7859–2.8235 ms/record, H 2.8077–2.8561 ms/record, and D 2.7447–2.7878 ms/record. On the same 12-record anchor slice, the GRU student arms S/H/D measured approximately 2.95–3.04 ms/record (affine 2.216–2.335 ms/record), also from batch-8 throughput. Native A1+A2 measured 205.613 ms/record on `public_base` and 206.517 ms/record on the target arm over 3 repeats × 12 records. The native and student paths have different algorithms and resource footprints, so these figures do not support an amortized speed-quality claim.

The student state is approximately 16.79 MB for affine and 25.98 MB for S/H/D. The shared FP32 embedding table is 1,050,673,488 bytes (1.047 s load plus 0.203 s device transfer). The prediction wrapper was attached from launch; its child returned 0, the student runner recorded 3,675,705,344 bytes peak RSS, and the watchdog recorded 3,371,618,304 bytes peak group RSS. The prediction watchdog ended `FAIL_CLOSED` because the leader's `/proc` status vanished during post-child cleanup, not because a resource threshold was observed; prediction GPU peak was unavailable. A separate training monitor attached late, after training had begun, and sampled whole-device rather than PyTorch reserved memory: maximum used 7,808,745,472 bytes, minimum free 8,945,401,856 bytes, host high-water 5,882,933,248 bytes, and maximum temperature 76°C. It too is supplementary and must not be treated as the prediction-run resource record. These are limitations on cost characterization, not reasons to reinterpret the accuracy result.
