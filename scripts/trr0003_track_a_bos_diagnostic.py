...
after the runner's known-BOS clamp for both the cached target and a target
generated by the same batch-one forward_full(input_ids) path.  The public
labels are used only to make a matched diagnostic input; evaluator-private
truth and the shared panel are never loaded.
"""
...
SCHEMA = "token-reconstruction.trr0003-track-a-bos-forward-diagnostic.v2"
VALIDATION_ROOT = Path("outputs/TRR-0003/track_b/public_validation_slice_v2")
DEFAULT_OUTPUT = Path("outputs/TRR-0003/track_a_bos_forward_diagnostic/public_bos_forward_v2.json")
...
def _evaluation_counts(iterations: int) -> dict[str, int | str]:
    if not isinstance(iterations, int) or isinstance(iterations, bool) or iterations <= 0:
        raise DiagnosticError("iterations must be positive for evaluation accounting")
    # Each inverse branch step calls the branch once for the proposal and
    # once again to measure the updated residual.  There are two branches per
    # layer and CUT_DEPTH layers.  The two targets each add two complete
    # cycle forwards; the forward-path comparison adds token and embedding
    # forwards.  These event classes are deliberately reported separately.
    inverse_branch_evaluations_per_target = iterations * 2 * 2 * CUT_DEPTH
    inverse_branch_calls = inverse_branch_evaluations_per_target * 2
    full_prefix_forward_passes = 6
    full_prefix_layer_evaluations = full_prefix_forward_passes * CUT_DEPTH
    return {
        "inverse_branch_evaluations_per_target": inverse_branch_evaluations_per_target,
        "inverse_branch_calls": inverse_branch_calls,
        "full_prefix_forward_passes": full_prefix_forward_passes,
        "full_prefix_layer_evaluations": full_prefix_layer_evaluations,
        "public_prefix_layer_evaluations": full_prefix_layer_evaluations,
        "note": "Branch calls and full-prefix layer evaluations are separate execution events and are not equivalent FLOPs.",
    }
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    try:
        ...
        for record_index in indices:
            ...
            forward, matched_batch1_target = _forward_record(
                input_ids=input_ids,
                cached=cached,
                state=state,
                embedding_weight=state.embedding_weight,
            )
            cached_cycle = _cycle_record(
                target=cached.float(),
                state=state,
                iterations=int(args.iterations),
                damping=float(args.damping),
            )
            matched_batch1_cycle = _cycle_record(
                target=matched_batch1_target.float(),
                state=state,
                iterations=int(args.iterations),
                damping=float(args.damping),
            )
            track_a.synchronize()
            counts = _evaluation_counts(int(args.iterations))
            records.append({
                ...
                "forward": forward,
                "cycle_targets": {
                    "cached_h4": cached_cycle,
                    "matched_batch1_token_forward": matched_batch1_cycle,
                },
                "inference_seconds": time.perf_counter() - record_started,
                "inverse_branch_evaluations_per_target": counts["inverse_branch_evaluations_per_target"],
                "inverse_branch_calls": counts["inverse_branch_calls"],
                "full_prefix_forward_passes": counts["full_prefix_forward_passes"],
                "full_prefix_layer_evaluations": counts["full_prefix_layer_evaluations"],
                "evaluation_events": counts,
                "public_prefix_layer_evaluations": counts["public_prefix_layer_evaluations"],
                "candidate_prefix_simulations": 0,
            })
            del input_ids, cached, matched_batch1_target
            track_a.synchronize()
        ...
        evidence = {
            ...
            "track": "track_a",
            "status": "COMPLETED",
            "diagnostic_role": "public auxiliary forward-path and known-BOS clamp contribution, with same-batch matched target control",
            ...
            "fixed_execution": {
                ...
                "matched_target_control": "target = public_prefix.forward_full(input_ids) from the same batch-one call used by the token forward comparison",
                "evaluation_event_accounting": "inverse has two branch calls per step per branch (8 branches); full-prefix control/cycle forwards are counted separately",
            },
            ...
            "aggregate": {
                ...
                "inverse_branch_evaluations_per_target": sum(int(row["inverse_branch_evaluations_per_target"]) for row in records),
                "inverse_branch_calls": sum(int(row["inverse_branch_calls"]) for row in records),
                "full_prefix_forward_passes": sum(int(row["full_prefix_forward_passes"]) for row in records),
                "full_prefix_layer_evaluations": sum(int(row["full_prefix_layer_evaluations"]) for row in records),
                "public_prefix_layer_evaluations": sum(int(row["public_prefix_layer_evaluations"]) for row in records),
                "candidate_prefix_simulations": 0,
                "evaluation_event_note": "Branch calls and full-prefix layer evaluations are separate execution events and are not equivalent FLOPs.",
            },
        }
