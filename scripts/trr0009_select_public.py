#!/usr/bin/env python3
"""Select the prospective TRR-0009 public panel after owner freeze.

This adapter is deliberately separate from the count-only TRR-0009 inventory.
It consumes the reviewed decision contract, the inherited TRR-0007 method
freeze, the mandatory TRR-0009 selected-method freeze, the identity-only
inventory, and the approved opaque exclusions before it reads public Arrow
rows.  It reuses the trusted TRR-0005 renderer, TRR-0006 identity
...
# Keep ``python3 scripts/trr0009_select_public.py ...`` runnable from the
...
EXCLUSION_SCHEMA = "token-reconstruction.trr0009-source-exclusions.v1"
RESERVATION_SCHEMA = "token-reconstruction.trr0009-opaque-source-sequence-reservation.v1"
SELECTION_STATUS = "FROZEN_TRR0009_SOURCE_SELECTION_NO_TRUTH"
EXCLUSION_STATUS = "PUBLIC_IDENTITY_EXCLUSIONS_COMPLETE_NO_TRUTH"
RESERVATION_STATUS = "READY_FOR_TRR0009_CAPTURE_HASH_ONLY"
SELECTION_SEED = 5005
SEQUENCE_TOKENS = 128
...
HIDDEN_SIZE = 2048
EXPECTED_RECORDS_BY_DOMAIN = {"pile": 128, "finance": 256}
SOURCE_RANGES = {"pile": [7000, 10000], "finance": [12000, 20000]}
TARGET_CONDITIONS = ("public_base", "public_lora_2601")
HEX64 = re.compile(r"^[0-9a-f]{64}$")
HEX40 = re.compile(r"^[0-9a-f]{40}$")
TASK_METHOD_FREEZE_SCHEMA = "token-reconstruction.trr0009-selected-method-freeze.v1"
TASK_METHOD_FREEZE_STATUS = "FROZEN_TRR0009_METHODS_BEFORE_SOURCE_SELECTION"
TASK_METHOD_FREEZE_DEFAULT = Path("experiments/TRR-0009/training/method_freeze.json")
TASK_METHOD_ORDER = (
    "unchanged_anchor",
    "continued_fixed_readout",
    "continued_adaptable_readout",
    "retained_reference",
)
TASK_METHOD_ROLES = {
    "unchanged_anchor": "unchanged_anchor",
    "continued_fixed_readout": "continued_fixed_readout",
    "continued_adaptable_readout": "continued_adaptable_readout",
    "retained_reference": "published_reference",
}


class SelectionError(RuntimeError):
    """Raised when a TRR-0009 selection or reservation cannot proceed safely."""
...
def _validate_planning_status(path: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    """Require the finalized identity inventory's approved P06 convention."""

    record = _file_record(path, description="TRR-0009 exclusion inventory")
    payload = _load_json(path, description="TRR-0009 exclusion inventory")
    if payload.get("task_id") != TASK_ID:
        raise SelectionError("exclusion inventory task identity changed")
    if payload.get("status") != "IDENTITY_INVENTORY_COMPLETE_NO_SELECTION_NO_TRUTH":
        raise SelectionError("source selection requires a finalized count-only inventory")
    compatibility = payload.get("p06_hash_compatibility")
    if not isinstance(compatibility, Mapping):
        raise SelectionError("exclusion inventory lacks P06 hash compatibility evidence")
    ...
    if status != "VERIFIED_P06_PRODUCER_CONFIRMATION":
        raise SelectionError(
            ...
            f"until source_inventory.p06_hash_compatibility.source_hash_byte_input_status="
            f"VERIFIED_P06_PRODUCER_CONFIRMATION (currently {status!r})"
        )
    algorithm = compatibility.get("sequence_hash_algorithm")
    if not isinstance(algorithm, str) or not algorithm.startswith("VERIFIED:"):
        raise SelectionError("P06 H128 sequence convention is not verified in source inventory")
    return record, dict(compatibility)


def _validate_decision_contract(path: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    record = _file_record(path, description="TRR-0009 decision contract")
    ...
    expected_methods = {
        "unchanged_anchor": "trr0009_unchanged_current_enriched__residual_mlp512",
        "continued_fixed_readout": "trr0009_continued_fixed_readout",
        "continued_adaptable_readout": "trr0009_continued_supported_token_gain_bias_readout",
        "retained_reference": "trr6__enriched_trained_diagonal_attention128",
    }
    if not isinstance(methods, Mapping) or any(methods.get(k) != v for k, v in expected_methods.items()):
        raise SelectionError("decision contract arm roles changed")
    ...
    if (
        ...
        or primary.get("cell") != "finance__public_base"
        or primary.get("route_alpha") != 0.025
        or primary.get("component_alpha") != 0.0125
        or primary.get("exact_point_floor") != 0.02
        or primary.get("token_point_floor") != 0.0025
    ):
        raise SelectionError("decision contract primary useful-pilot gates changed")
    ...
    if (
        ...
        or safeguards.get("all_four_cells_required") is not True
        or set(safeguards.get("cells", ())) != expected_cells
    ):
        raise SelectionError("decision contract safeguard gates changed")
    rare = payload.get("rare_token_safeguard")
    if (
        not isinstance(rare, Mapping)
        or rare.get("frequency_bins") != ["0", "1-4", "5-9", "10-49", "50+"]
        or rare.get("target_bins") != ["0", "1-4"]
        or rare.get("cells") != sorted(expected_cells)
        or rare.get("all_four_cells_required") is not True
        or rare.get("harm_margin") != 0.01
        or rare.get("minimum_exposed_sources") != 32
    ):
        raise SelectionError("decision contract rare-token safeguard changed")
    bootstrap = payload.get("bootstrap")
    if (
        not isinstance(bootstrap, Mapping)
        or bootstrap.get("seed") != 9009
    ...
    freeze = payload.get("truth_and_freeze")
    if not isinstance(freeze, Mapping) or freeze.get("source_selection") != (
        "starts only after this contract is owner-frozen and actual exclusion compatibility is reviewed"
    ):
        raise SelectionError("decision contract source-selection gate is malformed")
    provenance = payload.get("provenance")
    if not isinstance(provenance, Mapping):
        raise SelectionError("decision contract provenance is absent")
    for key in ("p06_underlying_provenance_opened", "p03_holdout_opened", "p07_workspace_or_results_opened"):
        if provenance.get(key) is True:
            raise SelectionError(f"decision contract records forbidden access: {key}")
    return record, payload


def _validate_task_method_freeze(
    path: Path, *, root: Path, decision_record: Mapping[str, Any]
) -> tuple[dict[str, Any], dict[str, Any], dict[str, dict[str, Any]]]:
    """Require actual TRR-0009 method states frozen before source selection.

    This freeze is separate from the inherited TRR-0007 method ledger and from
    the later prediction registration.  It binds the selected contender,
    continued control, adaptable arm, and retained reference states, the
    decision rules, and the exact source-code files used to produce them.
    """

    record = _file_record(path, description="TRR-0009 selected-method freeze")
    payload = _load_json(path, description="TRR-0009 selected-method freeze")
    if payload.get("schema") != TASK_METHOD_FREEZE_SCHEMA:
        raise SelectionError("TRR-0009 selected-method freeze schema changed")
    if payload.get("task_id") != TASK_ID:
        raise SelectionError("TRR-0009 selected-method freeze task identity changed")
    if payload.get("status") != TASK_METHOD_FREEZE_STATUS:
        raise SelectionError("TRR-0009 selected-method freeze is missing or not frozen")
    if payload.get("source_selection_started") is not False:
        raise SelectionError("TRR-0009 selected-method freeze records source selection started")
    for key in ("truth_opened", "private_or_truth_payload_read", "fresh_evaluation_started"):
        if payload.get(key) is True:
            raise SelectionError(f"TRR-0009 selected-method freeze records forbidden access: {key}")
    if payload.get("state_selection_frozen") is not True:
        raise SelectionError("TRR-0009 selected-method freeze does not certify frozen states")
    contract_binding = payload.get("decision_contract")
    if not isinstance(contract_binding, Mapping):
        raise SelectionError("TRR-0009 selected-method freeze lacks decision-contract binding")
    if contract_binding.get("sha256") != decision_record.get("sha256"):
        raise SelectionError("TRR-0009 selected-method freeze binds a different decision contract")
    decision_rules = payload.get("decision_rules")
    if not isinstance(decision_rules, Mapping):
        raise SelectionError("TRR-0009 selected-method freeze lacks decision rules")
    if (
        decision_rules.get("decision_contract_sha256") != decision_record.get("sha256")
        or decision_rules.get("selection_metric")
        != "earliest maximum public validation style-balanced token accuracy including step zero"
        or decision_rules.get("validation_interval") != 100
        or decision_rules.get("step_zero_eligible") is not True
        or decision_rules.get("same_selection_opportunities") is not True
    ):
        raise SelectionError("TRR-0009 selected-method freeze decision rules changed")
    method_order = payload.get("method_order")
    if tuple(method_order or ()) != TASK_METHOD_ORDER:
        raise SelectionError("TRR-0009 selected-method freeze method order changed")
    states = payload.get("state_bindings")
    if not isinstance(states, Mapping) or set(states) != set(TASK_METHOD_ORDER):
        raise SelectionError("TRR-0009 selected-method freeze lacks all actual method states")
    checked_states: dict[str, dict[str, Any]] = {}
    for method_id in TASK_METHOD_ORDER:
        row = states.get(method_id)
        if not isinstance(row, Mapping) or row.get("id") != method_id or row.get("role") != TASK_METHOD_ROLES[method_id]:
            raise SelectionError(f"TRR-0009 selected-method state role changed: {method_id}")
        state = row.get("state")
        if not isinstance(state, Mapping):
            raise SelectionError(f"TRR-0009 selected-method state is absent: {method_id}")
        state_path = Path(str(state.get("path", ""))).expanduser()
        if not state_path.is_absolute():
            state_path = root / state_path
        actual = _file_record(state_path, description=f"TRR-0009 {method_id} selected state")
        if actual.get("bytes") != state.get("bytes") or actual.get("sha256") != state.get("sha256"):
            raise SelectionError(f"TRR-0009 selected-method state binding changed: {method_id}")
        try:
            selected_step = int(row.get("selected_step"))
        except (TypeError, ValueError) as exc:
            raise SelectionError(f"TRR-0009 selected-method step is malformed: {method_id}") from exc
        if selected_step < 0:
            raise SelectionError(f"TRR-0009 selected-method step is negative: {method_id}")
        ...
        if not isinstance(loader, Mapping) or not isinstance(loader.get("module"), str) or not isinstance(loader.get("function"), str):
            raise SelectionError(f"TRR-0009 selected-method loader binding is absent: {method_id}")
        checked_states[method_id] = dict(actual) | {"selected_step": selected_step, "loader": dict(loader)}
    source_code = payload.get("source_code")
    if not isinstance(source_code, list) or not source_code:
        raise SelectionError("TRR-0009 selected-method freeze lacks source-code bindings")
    checked_code: list[dict[str, Any]] = []
    for index, item in enumerate(source_code):
        if not isinstance(item, Mapping):
            raise SelectionError(f"TRR-0009 selected-method source binding is malformed: {index}")
        source_path = Path(str(item.get("path", ""))).expanduser()
        if not source_path.is_absolute():
            source_path = root / source_path
        actual = _file_record(source_path, description=f"TRR-0009 selected-method source {index}")
        if actual.get("bytes") != item.get("bytes") or actual.get("sha256") != item.get("sha256"):
            raise SelectionError(f"TRR-0009 selected-method source binding changed: {index}")
        checked_code.append(dict(actual) | {"role": item.get("role")})
    code_commit = payload.get("code_commit")
    if not isinstance(code_commit, str) or HEX40.fullmatch(code_commit) is None:
        raise SelectionError("TRR-0009 selected-method freeze code commit is malformed")
    return record, payload, checked_states
...
def _validate_method_freeze(
    ...
    if not required.issubset(set(payload.get("method_ids", ()) )):
        raise SelectionError("method freeze lacks a required TRR-0009 method state")
...
def _validate_inventory(
    ...
    record = _file_record(path, description="TRR-0009 identity inventory")
    payload = _load_json(path, description="TRR-0009 identity inventory")
    if payload.get("schema") != "token-reconstruction.trr0009-source-inventory.v1":
    ...
    if payload.get("task_id") != TASK_ID or payload.get("status") != "IDENTITY_INVENTORY_COMPLETE_NO_SELECTION_NO_TRUTH":
        raise SelectionError("identity inventory is not a finalized count-only projection")
    ...
    for style in STYLE_ORDER:
        ...
        if not isinstance(available, int) or available < int(expected_counts[style]):
            raise SelectionError(f"identity inventory {style} has insufficient verified capacity")
    ...
    if not isinstance(execution, Mapping):
        raise SelectionError("identity inventory execution receipt is absent")
...
def _known_exclusion_paths(root: Path) -> list[Path]:
    ...
    paths.extend(
        ...
        for relative in (
            ...
            planning.TRR8_EXCLUSIONS,
            planning.TRR8_SELECTION,
        )
    )
    result: list[Path] = []
    seen: set[str] = set()
...
def _build_selection_payload(
    *,
    ...
    task_method_record: Mapping[str, Any],
    task_method_freeze: Mapping[str, Any],
    task_method_states: Mapping[str, Mapping[str, Any]],
    inventory_record: Mapping[str, Any],
    ...
    return {
        ...
        "trr0009_method_freeze": dict(task_method_record),
        "trr0009_method_freeze_sha256": task_method_record["sha256"],
        "trr0009_method_freeze_status": task_method_freeze.get("status"),
        "trr0009_method_freeze_code_commit": task_method_freeze.get("code_commit"),
        "trr0009_method_freeze_state_sha256": {
            method_id: state["sha256"] for method_id, state in task_method_states.items()
        },
        "identity_inventory": dict(inventory_record),
        ...
        "execution": {
            ...
            "selector_source": _file_record(selector_source, description="TRR-0009 selector source"),
...
def select_public(args: argparse.Namespace) -> dict[str, Any]:
    ...
    task_method_path = (
        (root / args.task_method_freeze).resolve()
        if not Path(args.task_method_freeze).is_absolute()
        else Path(args.task_method_freeze).resolve()
    )
    task_method_record, task_method_freeze, task_method_states = _validate_task_method_freeze(
        task_method_path, root=root, decision_record=decision_record
    )
    ...
    exclusion_record = _write_create_only(
        exclusions_path, exclusion_payload, description="TRR-0009 source exclusions"
    )
    selection_payload = _build_selection_payload(
        ...
        task_method_record=task_method_record,
        task_method_freeze=task_method_freeze,
        task_method_states=task_method_states,
        inventory_record=inventory_record,
        inventory=inventory,
    ...
    selection_record = _write_create_only(
        selection_path, selection_payload, description="TRR-0009 source selection"
    )
    return {
        "task_id": TASK_ID,
        "status": SELECTION_STATUS,
...
def _load_selection_for_reservation(path: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    record = _file_record(path, description="TRR-0009 source selection")
    payload = _load_json(path, description="TRR-0009 source selection")
...
def _reservation_payload(
    ...
    return {
        ...
        "created_utc": _utc_now(),
        "purpose": "hash-only TRR-0009 source and H128 sequence reservation",
...
def export_reservation(args: argparse.Namespace) -> dict[str, Any]:
    ...
    output_record = _write_create_only(output, payload, description="TRR-0009 opaque reservation")
...
def _parser() -> argparse.ArgumentParser:
    ...
    select.add_argument("--decision-contract", type=Path, default=Path("experiments/TRR-0009/planning/decision_contract.json"))
    select.add_argument("--planning-status", type=Path, default=Path("experiments/TRR-0009/planning/source_inventory.json"))
    select.add_argument("--inventory", type=Path, default=Path("experiments/TRR-0009/planning/source_inventory.json"))
    select.add_argument("--method-freeze", type=Path, default=Path("experiments/TRR-0007/method_freeze.json"), help="inherited TRR-0007 method freeze")
    select.add_argument("--task-method-freeze", type=Path, default=TASK_METHOD_FREEZE_DEFAULT, help="frozen TRR-0009 selected-method state binding")
    ...
    reserve.add_argument("--output", type=Path, default=Path("experiments/TRR-0009/selection/opaque_source_sequence_reservation.json"))
    return parser
...
if __name__ == "__main__":
    try:
        ...
        raise SystemExit(f"TRR-0009 selection error: {exc}")
