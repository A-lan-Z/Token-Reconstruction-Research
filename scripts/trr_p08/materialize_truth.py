#!/usr/bin/env python3
"""Materialize the private TRR-P08 evaluator truth after the joint freeze.
...
only a validated ``JOINT_FREEZE_VALIDATED_NO_TRUTH`` receipt permits reading the
...
from scripts.trr_p08 import capture_public as capture  # noqa: E402
from scripts.trr_p08 import score_frozen as scorer  # noqa: E402
...
TRUTH_FILE_SCHEMA = "token-reconstruction.trr-p08-private-truth-array.v1"
...
def _materialize_records(
    *,
    ...
    records = capture.p06_capture._materialize_selected(selected_rows, datasets=datasets, tokenizer=tokenizer)
...
def _resolve_record_path(record: Mapping[str, Any], *, root: Path, description: str) -> Path:
    ...
    if not isinstance(raw, str) or not raw:
        raise TruthMaterializationError(f"{description} path is missing")
...
def _bound_selection(*, root: Path, frozen: Mapping[str, Any]) -> tuple[Path, dict[str, Any], dict[str, Any]]:
    observation_path = _resolve_record_path(frozen["bindings"]["observation_manifest"], root=root, description="P08 observation manifest")
    observation = _load_json(observation_path, description="P08 observation manifest")
    source_pairing = observation.get("source_pairing")
    if not isinstance(source_pairing, Mapping) or not isinstance(source_pairing.get("record_ids_sha256"), Mapping):
        raise TruthMaterializationError("P08 observation manifest lacks source pairing")
    expected_ids = {domain: source_pairing["record_ids_sha256"].get(domain) for domain in DOMAINS}
    try:
        identity = scorer._artifact_selection_identity(
            observation_manifest_path=observation_path,
            observation_manifest=observation,
            root=root,
            expected_record_ids=expected_ids,
        )
    except (scorer.P08ScoreError, OSError, ValueError, TypeError) as exc:
        raise TruthMaterializationError(f"P08 source selection binding failed: {exc}") from exc
    selection_path = _resolve_record_path(identity["selection_record"], root=root, description="P08 source selection")
    selection = _load_json(selection_path, description="P08 source selection")
    return selection_path, selection, {"observation_path": observation_path, "observation": observation, "identity": identity}


def prepare_truth(args: argparse.Namespace) -> dict[str, Any]:
    if not args.execute:
        raise TruthMaterializationError("P08 truth materialization requires explicit --execute after joint freeze")
    ...
    # The only operation before evaluator inputs are touched is the complete
    # no-truth P08 validation.  It rehashes the fit states, observations, ties,
    # prediction files, and exact source-free prediction argv.
    try:
        frozen = scorer.validate_joint_freeze_artifacts(
            repository_root=root,
            freeze_receipt_path=freeze_path,
            expected_plan_sha256=args.expected_plan_sha256,
        )
    except (scorer.P08ScoreError, scorer.P08MetricsError, OSError, ValueError, TypeError) as exc:
        raise TruthMaterializationError(f"P08 joint freeze failed closed: {exc}") from exc

    selection_path, selection_meta, selection_context = _bound_selection(root=root, frozen=frozen)
    bound_selection = selection_context["identity"]["selection_record"]
    if args.source_selection is not None:
        requested_selection = Path(args.source_selection).expanduser().resolve()
        if requested_selection != selection_path or _sha256_file(requested_selection) != bound_selection["sha256"]:
            raise TruthMaterializationError("requested source selection differs from the P08 joint-freeze binding")

    source_universe = selection_meta.get("source_universe")
    if not isinstance(source_universe, Mapping):
        raise TruthMaterializationError("P08 source selection lacks its frozen source-universe binding")
    universe_path = Path(str(source_universe.get("path", ""))).expanduser().resolve()
    if args.universe is not None:
        requested_universe = Path(args.universe).expanduser().resolve()
        if requested_universe != universe_path:
            raise TruthMaterializationError("requested source universe differs from the P08 selection binding")
    if not universe_path.is_file() or _sha256_file(universe_path) != source_universe.get("sha256"):
        raise TruthMaterializationError("P08 source universe hash differs from the frozen selection")

    # Keep evaluator truth outside the reconstruction root and create-only.
    output_dir = _create_only_directory(output_dir_arg, repository_root=root)
    universe = capture.panel.load_universe(universe_path, require_frozen=True)
    capture._configure_capture(universe)
    source_paths = {
        domain: tuple(Path(value).expanduser().resolve() for value in _source_paths(selection_meta, domain))
        for domain in DOMAINS
    }
    ...
    expected_sequences = selection_context["identity"]["final_sequence_sha256"]
    ...
    manifest = {
        ...
        "source_selection_sha256": bound_selection["sha256"],
        "observation_manifest_sha256": bindings["observation_manifest"]["sha256"],
        "prediction_manifest_sha256": bindings["prediction_manifest"]["sha256"],
        "record_ids_sha256": dict(selection_context["identity"]["record_ids_sha256"]),
        ...
        "joint_freeze_source_commits": {
            "fit": frozen["validated"].get("fit_source_commit"),
            "prediction": frozen["validated"].get("prediction_source_commit"),
        },
        "selection": {
            "path": str(selection_path),
            "bytes": int(selection_path.stat().st_size),
            "sha256": bound_selection["sha256"],
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--expected-plan-sha256", default=scorer.freeze_matrix.APPROVED_PLAN_SHA256)
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"TRR-P08 truth materialization failed closed: {exc}", file=sys.stderr)
        return 2
