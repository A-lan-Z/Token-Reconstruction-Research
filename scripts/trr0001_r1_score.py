#!/usr/bin/env python3
"""Open R1 truth through every freeze/reveal gate and score frozen outputs."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--selection-reveal", type=Path, required=True)
    parser.add_argument("--selection-verification", type=Path, required=True)
...
def verify_public_gates_before_truth(
    args: argparse.Namespace, root: Path
) -> dict[str, Any]:
    """Verify frozen bytes, access evidence, and mapping reveal before truth."""
    ...
    if (
        metadata.get("revision_id") != "TRR-0001-R1"
        or metadata.get("access_manifests_verified") is not True
        or metadata.get("selection_revealed") is not False
        or metadata.get("truth_opened") is not False
    ):
        raise RuntimeError("freeze receipt metadata does not establish the R1 gate")
    expected_frozen = (root / receipt["frozen_root"]).resolve()
    if expected_frozen != args.frozen_root.resolve():
        raise RuntimeError("requested frozen root differs from the verified receipt")

    config = load_json(expected_frozen / "reconstructor_input/sanitized_config.json")
    observation_index = load_json(
        expected_frozen / "reconstructor_input/observation_index.json"
    )
    ...
    if set(reveal) != {"schema", "revealed_utc", "selection_key_hex", "records"}:
        raise RuntimeError("selection reveal fields changed")
    if not isinstance(reveal["records"], list) or len(reveal["records"]) != 64:
        raise RuntimeError("selection reveal coverage changed")
    if any(
        set(row) != {"record_id", "dataset_index", "text_sha256"}
        for row in reveal["records"]
    ):
        raise RuntimeError("selection reveal contains truth or unexpected fields")
    if [row["record_id"] for row in reveal["records"]] != config["record_order"]:
        raise RuntimeError("revealed mapping differs from frozen opaque order")
    verification = load_json(args.selection_verification)
    if (
        verification.get("result")
        != "PASS_SELECTION_REVEALED_AFTER_VERIFIED_FREEZE"
        or verification.get("exit_status") != 0
        or verification.get("truth_sidecar_read") is not False
        or verification.get("token_ids_disclosed_in_mapping_reveal") is not False
        or verification.get("access_manifests_reverified_before_private_mapping_read")
        is not True
        or verification.get("reveal", {}).get("sha256")
        != sha256_file(args.selection_reveal)
    ):
        raise RuntimeError("selection reveal verification is absent or inconsistent")
    return receipt
...
def main() -> int:
    ...
    with timer.measure("verify_freeze_access_and_selection_reveal_gate"):
        receipt = verify_public_gates_before_truth(args, root)
    with timer.measure("read_truth_after_all_successful_gates"):
        truth_rows = read_jsonl(args.truth)
    if len(truth_rows) != 64 or any(
        set(row) != {"record_id", "token_ids"} for row in truth_rows
    ):
        raise RuntimeError("truth schema or record count changed")
    ...
    direct_rows = read_jsonl(
        args.frozen_root / "direct_inverse/reconstructions.jsonl"
    )
    causal_rows = read_jsonl(
        args.frozen_root / "causal_public_surrogate_search/reconstructions.jsonl"
    )
    direct_route = load_json(args.frozen_root / "direct_inverse/route.json")
    causal_route = load_json(
        args.frozen_root / "causal_public_surrogate_search/route.json"
    )
    if (
        len(direct_rows) != 384
        or len(causal_rows) != 384
        or direct_route.get("truth_or_source_inputs") != 0
        or causal_route.get("truth_or_source_inputs") != 0
        or direct_route["record_order"] != causal_route["record_order"]
    ):
        raise RuntimeError("frozen reconstruction coverage or separation changed")
    if [row["record_id"] for row in truth_rows] != direct_route["record_order"]:
        raise RuntimeError("truth and frozen opaque record orders differ")
    frozen_rows = []
    for direct, causal in zip(direct_rows, causal_rows):
        identity = ("condition", "cut_depth", "record_index", "record_id")
        if any(direct[key] != causal[key] for key in identity):
            raise RuntimeError("frozen method row identities differ")
        if (
            direct["candidate_ids"] != causal["candidate_ids"]
            or direct["proposal_scores"] != causal["proposal_scores"]
        ):
            raise RuntimeError("frozen candidate proposals differ across processes")
        frozen_rows.append(
            {
                **{key: direct[key] for key in identity},
                "candidate_ids": direct["candidate_ids"],
                "direct_tokens": direct["prediction_tokens"],
                "causal_tokens": causal["prediction_tokens"],
                "direct_candidate_scores": direct["proposal_scores"],
                "causal_candidate_scores": causal["selection_scores"],
                "direct_proposal_seconds": direct["proposal_amortized_seconds"],
                "direct_selection_seconds": direct["selection_amortized_seconds"],
                "direct_total_seconds": direct["total_amortized_seconds"],
                "causal_proposal_seconds": causal["proposal_amortized_seconds"],
                "causal_selection_seconds": causal["selection_amortized_seconds"],
                "causal_total_seconds": causal["total_amortized_seconds"],
            }
        )
    row_by_arm_record = {
    ...
    with timer.measure("verify_opened_truth_against_revealed_dataset_mapping"):
        dataset = load_dataset(
            DATASET_ID, revision=DATASET_REVISION, split="train"
        )
        reveal_rows = load_json(args.selection_reveal)["records"]
        for mapping, truth in zip(reveal_rows, truth_rows):
            text = str(dataset[int(mapping["dataset_index"])]["text"])
            expected_tokens = [
                BOS_TOKEN_ID,
                *tokenizer(text, add_special_tokens=False)["input_ids"][:39],
            ]
            if (
                hashlib.sha256(text.encode("utf-8")).hexdigest()
                != mapping["text_sha256"]
                or expected_tokens != truth["token_ids"]
            ):
                raise RuntimeError("opened truth differs from revealed source mapping")
        del dataset
    device = next(model.parameters()).device
    embedding_table = normalized_embeddings(
        model.get_input_embeddings().weight
    ).to(device)
    query_state = load_file(
        args.frozen_root / "direct_inverse/queries.safetensors", device="cpu"
    )
    causal_query_state = load_file(
        args.frozen_root / "causal_public_surrogate_search/queries.safetensors",
        device="cpu",
    )
    if set(query_state) != set(causal_query_state) or any(
        not torch.equal(query_state[key], causal_query_state[key])
        for key in query_state
    ):
        raise RuntimeError("frozen method query tensors differ")
    expected_query_keys = {
    ...
    observation_index_path = (
        args.frozen_root / "reconstructor_input/observation_index.json"
    )
    observation_index = load_json(observation_index_path)
    entries = {
    ...
    with timer.measure("teacher_prefix_public_surrogate_counterfactual"):
        for condition in CONDITIONS:
            for cut in CUT_DEPTHS:
                ...
                observation_path = observation_index_path.parent / entry["path"]
                if sha256_file(observation_path) != entry["sha256"]:
    ...
    frequency_payload = load_json(args.frequency_counts)
    frequency_counts = {
                ...
                for method, prediction in method_predictions.items():
                    ...
                    row = {
                        ...
                        **{key: value for key, value in measured.items() if key not in ("correctness", "true_in_top16")},
                        ...
                        "proposal_amortized_seconds": frozen[
                            f"{timing_prefix}_proposal_seconds"
                        ],
                        "selection_amortized_seconds": frozen[
                            f"{timing_prefix}_selection_seconds"
                        ],
                        "amortized_seconds": frozen[
                            f"{timing_prefix}_total_seconds"
                        ],
                    }
    ...
    direct_evidence = load_json(
        args.frozen_root / "direct_inverse/reconstructor_evidence.json"
    )
    causal_evidence = load_json(
        args.frozen_root
        / "causal_public_surrogate_search/reconstructor_evidence.json"
    )
    method_evidence = {
        "direct_inverse": direct_evidence,
        "causal_public_surrogate_search": causal_evidence,
    }
    cost_by_method = {}
    for method, evidence in method_evidence.items():
        timings = evidence["arm_timings"]
        cost_by_method[method] = {
            "proposal_seconds": sum(row["proposal_seconds"] for row in timings),
            "selection_seconds": sum(row["selection_seconds"] for row in timings),
            "method_compute_seconds": evidence["method_compute_seconds"],
            "end_to_end_arm_seconds": sum(
                row["end_to_end_arm_seconds"] for row in timings
            ),
            "method_compute_seconds_per_record_arm": evidence[
                "method_compute_seconds_per_record_arm"
            ],
            "method_compute_seconds_per_scored_token": evidence[
                "method_compute_seconds_per_scored_token"
            ],
            "embedding_comparisons": evidence["embedding_comparisons"],
            "candidate_simulations": evidence["candidate_simulations"],
            "target_prefix_calls": evidence["target_prefix_calls"],
            "public_surrogate_model_loads": evidence["public_surrogate_model_loads"],
            "persisted_method_state_bytes": sum(
                entry["bytes"] for entry in evidence["persisted_method_state"]
            ),
            "memory": evidence["memory"],
            "implementation_complexity": evidence["implementation_complexity"],
        }
    metrics = {
        "schema": "token-reconstruction.trr0001-r1-metrics.v1",
        "task_id": "TRR-0001",
        "revision_id": "TRR-0001-R1",
        "truth_opened_after_verified_freeze": True,
        "selection_mapping_revealed_and_verified_before_truth": True,
        "opened_truth_matches_revealed_dataset_mapping": True,
        ...
        "cost": {
            "methods": cost_by_method,
            "target_prefix_calls": 0,
            "candidate_budget": 16,
            "frozen_bundle_bytes": sum(
                entry["bytes"] for entry in receipt["entries"]
            ),
            "scoring_peak_memory": peak_memory(),
            "cost_accounting": (
                "Each method reports its own proposal, selection, total compute, "
                "memory, evaluation counts, persisted state, and implementation "
                "complexity; causal cost includes proposal plus selection."
            ),
        },
    ...
    diagnostics = {
        "schema": "token-reconstruction.trr0001-r1-diagnostics.v1",
        "task_id": "TRR-0001",
        "revision_id": "TRR-0001-R1",
    ...
    evidence = {
        "schema": "token-reconstruction.trr0001-r1-score-evidence.v1",
        "task_id": "TRR-0001",
        "revision_id": "TRR-0001-R1",
        ...
        "truth_gate": {
            ...
            "access_manifests_reverified": True,
            "selection_mapping_revealed_before_truth": True,
            "receipt": file_record(args.receipt, root=root),
            "selection_reveal": file_record(args.selection_reveal, root=root),
            "selection_verification": file_record(
                args.selection_verification, root=root
            ),
        ...
        "frequency_counts": file_record(args.frequency_counts, root=root),
