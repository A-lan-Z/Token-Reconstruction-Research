...
def main(a):
    ...
    lines+=['','Stage 0 is the unchanged public base; stage 64 is the first fitted compatible LoRA descendant; stages 128 and 256 continue that same prefix-changing trajectory. This tests one bounded adaptation from the public base, not an independently released full-SFT descendant.','',
    ...
    for c in r['stage1_decision']:lines.append(f"| {c['domain']} | {c['stage']} | {c['smallest'] if c['smallest'] is not None else 'none among 8/16/32'} |")
    ...
    lines+=['', 'At K=8, Pile omitted 3, 2, 3 and 3 correct tokens at stages 0, 64, 128 and 256 respectively, affecting two clips each time. Finance omitted one token in one base clip and none at later snapshots. K=16 and K=32 omitted zero tokens in this panel. K=8 therefore fails the predeclared whole-clip limit despite its high token recall.', '', 'A high top-1 average also hid substantial clip errors: at the final Pile snapshot B1 top-1 accuracy was 99.237%, but only 20/32 clips were completely correct. K=8 raised this to 30/32, and K=16 to 32/32 fixed-list coverage. The smallest empirically viable shortlist did not grow along this trajectory.', '', 'The final boundary activations changed materially from base: mean per-record relative L2 was 0.348 for Pile and 0.517 for Finance. No shortlist collapse was observed through the tested updates, but this single bounded LoRA trajectory does not establish robustness to arbitrary fine-tuning.']
