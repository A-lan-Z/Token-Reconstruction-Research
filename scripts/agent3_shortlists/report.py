...
def main(a):
    ...
    lines+=['',f"Total cell wall (including loading): {sum(c['total_seconds'] for c in receipts):.3f}s. Sum of shared package/observation loading: {sum(c['load_seconds'] for c in receipts):.3f}s. Maximum process peak RSS: {max(c['peak_rss_bytes'] for c in receipts)/2**30:.3f}GiB. B1 record forwards:256; A1 record forwards:256; A2 candidate simulations:0.",'',
