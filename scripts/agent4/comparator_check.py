...
torch.set_num_threads(2);prefix=load_prefix(torch.bfloat16,device=args.device)
if args.device=='cuda':guard()
ids=torch.tensor([[128000,2028,374,264,1296]],device=args.device);obs=prefix.forward_full(ids).cpu()
...
native=decode_policy(observations=obs,attention_mask=mask,position_ids=positions,candidates=proposals.candidates,a1_confidence=proposals.top1_confidence,precut=prefix,device=torch.device(args.device),policy=policy,record_batch_size=1)
port=decode(prefix,obs[0],lens,embeddings,guard=guard if args.device=="cuda" else None)
...
write(EVID/f'comparator_equivalence_{args.device}.json',{'native_predictions':native.predictions[0].tolist(),'port_predictions':port['tokens'],'same_candidates_k256':True,'same_predictions':True,'native_policy_id':policy.policy_id,'environment':environment(),'peak_allocated':torch.cuda.max_memory_allocated() if args.device=="cuda" else 0,'peak_reserved':torch.cuda.max_memory_reserved() if args.device=="cuda" else 0})
