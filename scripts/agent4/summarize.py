...
def residual_stats(x):
    ...
    return result
analysis={'matched_runtime_ratio_prefix_to_a1a2':ratio,'matched_residuals':residual_stats(p),'static_residuals':residual_stats(sp)}
...
Our finite128-trial budget does not inherit the paper's vocabulary-spanning recovery guarantee.
The initial SGD/snap procedure failed a one-token public CPU diagnostic after128 checks.
The one corrective variant uses Adam(lr0.01), a float32 provisional embedding cast to BF16 at the input, raw Euclidean vocabulary projection, no snaps/restarts, and at most128 checks/gradient steps per token. It found that diagnostic token in17checks. Model weights stay frozen; only BOS and its own earlier commitments are known. It uses only the current observed activation, commits the best checked MSE candidate on exhaustion, and preserves timed-out suffixes as abstentions.

The source model is public Llama-3.2-1B-Instruct revision9213176726f574b556790deb65791e0c5aa438b6; boundary is after blocks0–3. Forward input embeddings retain their original magnitudes. Full own-prefix recomputation avoids candidate-cache mutation but adds repeated computation. A1+A2 uses its native cached candidate helper and stable top512 proposal, first256 candidates, direct cosine and own prior commitments. Its CPU geometry port passed ordered-candidate and prediction equivalence against native `decode_policy` on a public fixture.

## Frozen final measurements

Four public-domain ordinary-text clips and two generated identifier clips, each16positions including BOS, were selected after settings freeze. They are unused within this task; this is not a canonical or repository-wide fresh benchmark. All90post-BOS tokens remain in the matched denominator. BF16 capture and reconstruction ran on AMD Ryzen9 9950X3D with two CPU threads. The GPU remained leased to Agent2; this pilot did not use it.
...
text+='\nFirst-error positions are zero-based with BOS at0. Later errors are reported after wrong commitments; that ordering alone does not establish their individual causes.\n\n'
...
No validated online model-prefix recovery implementation/state was located in the pinned resources. Agent2 was asked for qualified assets and provenance; no such recovery state was integrated. Consequently, the required public-versus-recovered comparison and own-history closed-loop Stage3 were not run. No cold-start tracking claim is made.
