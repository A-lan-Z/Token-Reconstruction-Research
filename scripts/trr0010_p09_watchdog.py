...
def _has_live_process_group_member(pgid: int, *, ignored_pids: set[int] | None = None) -> bool:
    ...
    process state, rather than RSS, so a missing VmRSS on the already reaped
    leader does not become a false live-resource failure.  Only the explicitly
    ignored leader PID may be omitted; any other live descendant keeps the
    guard fail-closed.
    """

    ignored_pids = set(ignored_pids or ())
    proc = Path("/proc")
    try:
        entries = list(proc.iterdir())
    except OSError as exc:
...
def main(argv: list[str] | None = None) -> int:
    ...
    command_record = {
        "schema": "token-reconstruction.trr0010-resource-watchdog-command.v1",
        "task_id": TASK_ID,
    ...
    time_receipt = {
        "schema": "token-reconstruction.trr0010-resource-watchdog-time.v1",
        "task_id": TASK_ID,
        "status": guard_status,
    ...
    finish = {
        "schema": "token-reconstruction.trr0010-resource-watchdog-finish.v1",
        "task_id": TASK_ID,
        "status": guard_status,
        "wrapper_exit_code": wrapper_exit,
        "child_return_code": child_returncode,
...
if __name__ == "__main__":
    try:
        ...
        print(f"TRR-TRR-0010 resource watchdog failed before launch: {exc}", file=sys.stderr)
