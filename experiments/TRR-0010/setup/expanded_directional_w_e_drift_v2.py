#!/usr/bin/env python3
"""CPU-only integrity summary for the selected expanded directional readout (v2).
...
and support IDs.  It also reports row cosine/angle, orthogonal delta norm,
and exact FP32 reconstruction counts so L2 drift is not misread as rotation.
It does not load observations, labels, source text, truth, or run an inference
path.
"""
...
def main() -> int:
    ...
    with safe_open(str(embedding), framework="pt", device="cpu") as e_file, safe_open(
        ...
        supported_w_l2: list[torch.Tensor] = []
        supported_cosine: list[torch.Tensor] = []
        supported_angle_rad: list[torch.Tensor] = []
        supported_orthogonal_l2: list[torch.Tensor] = []
        reconstruction_max_abs: list[torch.Tensor] = []
        reconstruction_l2: list[torch.Tensor] = []
        fp32_reconstruction_max_abs: list[torch.Tensor] = []
        fp32_reconstruction_l2: list[torch.Tensor] = []
        fp32_reconstruction_exact = 0
        nonzero_supported = 0
        ...
        for start in range(0, e_shape[0], args.chunk_rows):
            ...
            if supported_offsets:
                ...
                offset_tensor = torch.tensor(supported_offsets, dtype=torch.int64)
                positions = torch.tensor([support_index[token_id] for token_id in supported_rows], dtype=torch.int64)
                e_supported = e_rows[offset_tensor]
                w_supported = w_rows[offset_tensor]
                e_supported64 = e_supported.to(dtype=torch.float64)
                w_supported64 = w_supported.to(dtype=torch.float64)
                observed_delta = drift[offset_tensor]
                ...
                e_norm = torch.linalg.vector_norm(e_supported64, dim=1)
                w_norm = torch.linalg.vector_norm(w_supported64, dim=1)
                dot = (e_supported64 * w_supported64).sum(dim=1)
                cosine = torch.clamp(dot / (e_norm * w_norm), min=-1.0, max=1.0)
                projection = (observed_delta * e_supported64).sum(dim=1) / (e_norm * e_norm)
                orthogonal = observed_delta - projection.unsqueeze(1) * e_supported64
                supported_w_l2.append(w_norm)
                supported_cosine.append(cosine)
                supported_angle_rad.append(torch.acos(cosine))
                supported_orthogonal_l2.append(torch.linalg.vector_norm(orthogonal, dim=1))
                fp32_expected_w = e_supported + delta_rows[positions]
                fp32_error = (w_supported - fp32_expected_w).to(dtype=torch.float64)
                fp32_reconstruction_max_abs.append(fp32_error.abs().amax(dim=1))
                fp32_reconstruction_l2.append(torch.linalg.vector_norm(fp32_error, dim=1))
                fp32_reconstruction_exact += int((fp32_error.abs().amax(dim=1) == 0).sum().item())
    ...
    payload = {
        "schema": "token-reconstruction.trr0010-expanded-directional-w-e-drift.v2",
        "task_id": "TRR-0010",
        ...
        "supported_w_row_l2": _stats(supported_w_l2),
        "supported_row_cosine_e_w": _stats(supported_cosine),
        "supported_row_angle_rad_e_w": _stats(supported_angle_rad),
        "supported_orthogonal_delta_l2": _stats(supported_orthogonal_l2),
        ...
        "fp32_e_plus_delta_equals_w": {
            "exact_rows": fp32_reconstruction_exact,
            "checked_rows": int(support_ids.numel()),
            "max_abs_error": _stats(fp32_reconstruction_max_abs),
            "l2_error": _stats(fp32_reconstruction_l2),
        },
        "interpretation": "Only supported token rows changed; unsupported rows are exact W==E rows. Supported row cosine/angle and orthogonal delta quantify rotation separately from norm drift; W-E matches stored delta_rows within serialized float arithmetic.",
