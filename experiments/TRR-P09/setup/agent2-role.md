...
The unchanged published TRR-0009 state is the fixed-readout starting reference. P09 keeps A2 out of deployed inference and does not add contextual access, staging, teacher ranking, routing, or ensembles. Public fitting material remains separate from final evaluation material. Public fitting H, token labels, masks, and hash-bound bank metadata are authorized inputs to student/control preparation; current/final evaluation plaintext, source-token rows, target labels, and truth payloads never enter the public fitting bank or its student/control metadata; Agent 1 owns post-freeze scoring.
