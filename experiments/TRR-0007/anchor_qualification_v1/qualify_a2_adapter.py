#!/usr/bin/env python3
"""Bounded one-row TRR-0007 A1+A2 adapter qualification.
    ...
    for path, method_id in ((A1_PATH, "historical_alpaca_a1"), (A2_PATH, "frozen_a1_a2_k256")):
        with safe_open(str(path), framework="pt", device="cpu") as handle:
            if set(handle.keys()) != {"predictions"}:
                raise QualifierError(f"prediction fixture keys changed: {path}")
...
def main() -> int:
    ...
    try:
        ...
        peak_reserved = None
    receipt = {
        "schema": "token-reconstruction.trr0007-a2-adapter-qualification.v1",
        "task_id": TASK_ID,
        "status": status,
