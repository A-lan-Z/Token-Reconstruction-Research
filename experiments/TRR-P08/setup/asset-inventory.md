...
The proposed comparison has four logical arms and eight seeded fits: positionwise and past-only, each with joint and affine-then-complete fitting under seeds 6106 and 6107. All arms must use the same public fit and validation roles, H128 geometry, readout vocabulary, and total 3,000-update exposure; the staged schedule is 1,000 affine-only updates followed by 2,000 complete-decoder updates. The staged affine phase is fully charged toward that budget, with no optional anchor in the current frozen design.

A modest fresh panel is proposed as 256 paired source records per Finance and Pile domain under public_base and public_lora_2601, but no records are selected. Selection requires a frozen source universe, stable ordering, and the root-owned approved hash-only ledgers. P06/P07 and the published TRR-0006 subset are recorded as hash-only exclusions. The exact approved TRR-0008 opaque export was verified by path, byte count, SHA, and schema; no other TRR-0008 or any TRR-0009 file was read. P03 remains sealed. Universal disjointness is not claimed.
